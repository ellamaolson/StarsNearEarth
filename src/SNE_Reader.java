...
public class SNE_Reader {
	SNE_Array_List<SNE> stars = new SNE_Array_List<SNE>();
	private int numStars;
	private boolean fileOpenError;
	private String starFile;

	// accessors
	public SNE getStar(int k)
	{
		if (k < 0 || k >= numStars)
			return new SNE();  // dummy return
		return stars.get(k);
	}
	...
	// helper
	private SNE readOneStar(String line)
	{
		int endOfField;
		SNE star = new SNE();
	...
	// constructor
	public SNE_Reader(String fileName)
	{
		int k;
		SNE star;
		...
		{  
			...
			// for each line that starts #. read and add to SNE_Array_List
